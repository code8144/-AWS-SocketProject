	...
	public void run() {
		try {
			...
			while(true) {
				...
				RequestDto requestDto = gson.fromJson(request, RequestDto.class);	// request를 원래 형태(RequestDto)로 변환후 requestDto 에 저장
				...
				switch(requestDto.getResource()) {	// requestDto의 Resource를 가져와서 case별로 실행
					case "join" : 
						...
						userName = joinReqDto.getUsername();	// 오류검사에 걸리지 않으면 가져온 joinReqDto의 userName을 connectedSocket의 userName에 넣어줌
						
						// ResponseDto의 resource(joinSuccess), status("ok"), body(접속 성공!)에 저장후, joinSuccessResponseDto에 넣어줌
					...
					case "create" : 
						...
						Room room = new Room(roomName, userName);//Room 객체를 생성하여 매개변수에 roomName(requestDto.getBody())과 kingName(userName)을 넣음
...
//				}
	...
	private void reflashRoomList() {
		...
		// responseDto의 resource(reflashRoom), status(ok), body(gson.toJson(roomNameList))를 넣음
	...
	// ResponseDto 객체를 JSON 형태로 변환하여, 주어진 ConnectedSocket 목록에 속한 소켓들에게 해당 JSON 데이터를 전송하는 기능을 수행하는 메소드
